...
def _plot_heat_map_p_trans(
    p_trans, transition, data_type, v_min, v_max, prm, colourbar=False
):
    if prm["plots"]:
        fig = plt.figure()
        sns.heatmap(
            p_trans[data_type][transition],
            vmin=v_min,
            vmax=v_max,
            cmap="RdBu_r",
            yticklabels=False,
...
def _get_profs(
    ...
    for k, bank_ in bank.items():
        if data_type == "car" and k == n_k - 1:
            ...
            assert all(
                sum(prof) == 0 or abs(sum(prof) - 1) < 1e-3 for prof in bank_["profs"]
...
class Clusterer:
        ...
        for data_type in self.behaviour_types:
            ...
            for i in range(n_data_type[data_type]):
                ...
                if data_type == "car":
                    ...
                    day_number = (
                        datetime(2010, 1, 1) + timedelta(delta_days)
                    ).weekday()
    ...
    def _initialise_cluster_transition_dicts(self, n_consecutive_days):
        ...
        for info in ["n_trans", "p_trans"]:
            ...
            for data_type in self.behaviour_types:
                self.__dict__[info][data_type] = {
                    day_trans: [] for day_trans in self.day_trans
                }

                for transition in self.day_trans:
                    for i in range(n_consecutive_days):
                        self.banks[data_type][transition][
                            f"f{i}of{n_consecutive_days}"
                        ] = []
    ...
    def _cluster_data_and_day_type(self, data_type, day_type, days):
        ...
        if len(transformed_features) < 2:
            print(
                f"len(transformed_features) {len(transformed_features)} "
        ...
        labels, cluster_distance, days[f"{data_type}_{day_type}"] = (
            self._cluster_module(
        ...
        banks_ = self._get_p_clus(
            vals_k, day_type, data_type, self.n_zeros[data_type][day_type]
        )
        banks_ = _get_profs(
            to_cluster, banks_, days_, data_type, idx_k_clustered, cluster_distance
        )
        ...
        [
            ...
            self.clus_dist_bin_edges[data_type][day_type],
            self.clus_dist_cdfs[data_type][day_type],
        ] = self._get_cdfs(distances, f"{data_type} {day_type}", banks_)
        (
            statistical_indicators,
            statistical_indicators_test,
            statistical_indicators_train,
        ) = self._plot_clusters(
    ...
    def _get_features(self, days_, data_type, day_type):
        ...
        if data_type == "car":
            i_zeros = [
                i
                for i in range(len(days_))
                if days_[i][data_type] is None or sum(days_[i][data_type]) == 0
            ]
        ...
        for i in to_cluster:
            ...
            if data_type == "loads":
                ...
                values = [
                    np.mean(
                        norm_day[
                            int(interval[0] * self.n / 24) : int(
                                interval[1] * self.n / 24
                            )
                        ]
                    )
                ...
                start_idx, end_idx = [
                    int(hour * 60 / self.prm["step_len"]) for hour in [6, 22]
                ]
    ...
    def _elbow_method(self, transformed_features, data_type, day_type):
        ...
        file = (
            self.prm["save_other"]
            / "clusters"
            / f"Elbow_method_{data_type}_{day_type}.npy"
        )
        if not file.is_file():
            wcss = []
            ...
            if self.prm["plots"]:
                ...
                plt.ylabel("WCSS")
                fig_save_path = (
                    self.prm["save_other"] / "clusters" / title.replace(" ", "_")
                )
    ...
    def _get_cdfs(self, distances, label, bank):
        ...
        for i, distance in enumerate(distances):
            ...
            if self.prm["plots"]:
                ...
                plt.hist(
                    distance, 100, density=True, facecolor="k", alpha=0.75, label="data"
                )
            ...
            all_cdfs = [
                cdfs[np.where(dist >= bins_edges[:-1])[0][-1]] for dist in distance
            ]
            ...
            if len(data[k]) > 0:
                ...
                for time in range(self.n):
                    for percentile in [10, 25, 50, 75, 90]:
                        statistical_indicators[k][f"p{percentile}"][time] = (
                            np.percentile(data[k][:, time], percentile)
                        )
                    statistical_indicators[k]["mean"][time] = np.mean(data[k][:, time])
                fig = plt.figure()
                plt.plot(statistical_indicators[k]["mean"], label="mean")
                fig.savefig(
                    self.prm["save_other"] / "clusters" / f"mean_{k}{label}.png"
                )
                plt.close("all")
            else:
    ...
    def _plot_clusters(
        ...
        train_vals_k = {
            k: vals_k[k][0 : int(len(vals_k[k]) * self.prm["train_set_size"])]
            for k in vals_k
        }
        test_vals_k = {
            k: vals_k[k][int(len(vals_k[k]) * self.prm["train_set_size"]) :]
            for k in vals_k
        }
        statistical_indicators_test = self._get_percentiles(test_vals_k, label="_test")
        statistical_indicators_train = self._get_percentiles(
            train_vals_k, label="_train"
        )
    ...
    def _group_gen_month(self, days_):
        ...
        for i_month, month in enumerate(range(1, 14)):
            ...
            self.banks["gen"][i_month]["profs"] = np.array(
                [days_[i]["norm_gen"] for i in i_days]
            )
            self.banks["gen"][i_month]["gen"] = np.array(
                [days_[i]["gen"] for i in i_days]
            )
            for property_ in ["factor", "id", "cum_day"]:
                self.banks["gen"][i_month][property_] = [
                    days_[i][property_] for i in i_days
                ]
            ...
            if len(self.banks["gen"][i_month]["profs"]) > 0:
                ...
                cluster_distances.append(
                    fitted_kmeans_obj.transform(transformed_features)
                )
            else:
        ...
        [
            ...
            self.min_cdfs["gen"],
            self.max_cdfs["gen"],
            self.clus_dist_bin_edges["gen"],
            self.clus_dist_cdfs["gen"],
        ] = self._get_cdfs(cluster_distances, "gen", self.banks["gen"])

        for i_month in range(13):
            np.save(
                self.prm["save_other"] / "clusters" / f"cdfs_clus_gen_{i_month}",
    ...
    def _transition_probabilities(
        ...
        self._get_n_trans(n_data_type, data_type, days, n_consecutive_days)
        if (
            data_type == "loads"
            and len(self.banks["loads"]["wd2wd"][f"f0of{n_consecutive_days}"]) == 0
        ):
            print(
                f"loads wd2wd f0of{n_consecutive_days}: len banks is 0 after _get_n_trans"
            )
                ...
                if n_consecutive_days == 2:
                    for transition in self.day_trans:
                        self.p_trans[data_type][transition][c0, c1] = (
                            ...
                            if sum(self.n_trans[data_type][transition][c0]) > 0
                            else None
                        )
                    ...
                    for c2 in range(n_clus_all_):
                        for transition in self.day_trans:
                            self.p_trans[data_type][transition][c0, c1, c2] = (
                                ...
                                if sum(self.n_trans[data_type][transition][c0, c1]) > 0
                                else None
                            )
                else:
        ...
        with open(
            self.prm["save_other"]
            / "clusters"
            / f"{data_type}_p_trans_n_consecutive_days{n_consecutive_days}",
            "wb",
        ) as file:
        ...
        if n_consecutive_days == 2:
            ...
            for day_type in self.weekday_types:
                pcluss = [
                    self.banks[data_type][day_type][k]["p_clus"]
                    for k in range(n_clus_all_)
                ]
    ...
    def _get_p_clus(self, vals_k, day_type, data_type, n_zeros):
        ...
        self.n_clus_all[data_type] = n_clus if data_type == "loads" else n_clus + 1
    ...
    def _get_n_trans(self, n_data_type, data_type, days, n_consecutive_days=2):
        ...
        for i in range(n_data_type[data_type] - (n_consecutive_days - 1)):
            consecutive_days = [
                days[data_type][d] for d in range(i, i + n_consecutive_days)
            ]
            same_id = all(
    ...
    def _generate_gan_profiles_behaviour_type(
        ...
        for k in range(self.prm["n_clus"][data_type]):
            if data_type == "car":
                percentage_car_avail = np.sum(ev_avail_k[k]) / np.multiply(
                    *np.shape(ev_avail_k[k])
                )
            ...
            generators[k], params[k] = compute_profile_generators(
                ...
                day_type,
                self.prm,
                percentage_car_avail,
                average_non_zero_trip,
            )
    ...
    def clustering(self, days, n_data_type):
        ...
        if "gen" in self.data_types:
            ...
            vals_k = {
                i_month: np.array(self.banks["gen"][i_month]["profs"])
                for i_month in range(13)
            }
            for i_month in [12]:
            ...
            # statistical_indicators = self._get_percentiles(vals_k)
            train_vals_k = {
                i_month: vals_k[i_month][
                    0 : int(len(vals_k[i_month]) * self.prm["train_set_size"])
                ]
                for i_month in vals_k
            }
            test_vals_k = {
                i_month: vals_k[i_month][
                    int(len(vals_k[i_month]) * self.prm["train_set_size"]) :
                ]
                for i_month in vals_k
            }
            statistical_indicators_test = self._get_percentiles(
                test_vals_k, label="_test"
            )
            statistical_indicators_train = self._get_percentiles(
                train_vals_k, label="_train"
            )
        ...
        if (
            "loads" in self.data_types
            and len(self.banks["loads"]["wd2wd"][f"f0of{n_consecutive_days}"]) == 0
        ):
            print(
                f"len(self.banks['loads']['wd2wd'][f0of{n_consecutive_days}]) == 0 in clustering"
            )
