...
class HEDGE:
    ...
    def _load_input_data(
        self, prm, other_prm, factors0, clusters0, brackets_definition
    ):
    ...
    def make_next_day(
        self, homes: Optional[list] = None, plotting: bool = False
    ) -> dict:
        ...
        for data_type in self.data_types:
            ...
            for i_home, home in enumerate(homes):
                cluster = (
                    clusters[data_type][home]
                    if data_type in self.behaviour_types
                    else self.date.month - 1
                )
                generated_profile = self._generate_profile(
                    data_type, day_type_, cluster
                )
                ...
                assert (
                    sum(generated_profile) == 0
                    or abs(sum(generated_profile) - 1) < 1e-3
        ...
        if any(
            factor > self.car["max_daily_energy_cutoff"] for factor in factors["car"]
        ):
    ...
    def _load_inputs(self, prm, brackets_definition):
        ...
        if not (Path(prm["paths"]["hedge_inputs_folder"]) / inputs_folder).exists():
            ...
            print(
                "Using sample data. See README for how to prepare the total input data."
            )
        ...
        for property_ in [
            ...
            potential_paths = utils.list_potential_paths(
                ...
                data_folder="hedge_inputs",
                sub_data_folder="clusters",
            )
            file_found = False
        ...
        if "gen" in self.data_types:
            self.residual_distribution_prms["gen"] = list(
        ...
        if self.factors0 is None:
            ...
            if "gen" in self.data_types:
                self.factors["gen"] = [
                    ...
                    + norm.ppf(
                        np.random.rand(), *self.residual_distribution_prms["gen"][1:]
                    )
                    for _ in self.homes
                ]
                ...
                self.factors["gen"] = np.minimum(
                    np.maximum(
                        self.f_min["gen"][i_month], np.array(self.factors["gen"])
                    ),
            ...
            if "car" in self.data_types:
                ...
                intervals = [
                    self._ps_rand_to_choice(
                        self.p_zero2pos["car"][transition], randoms[home]
                    )
                    for home in self.homes
                ]
                self.factors["car"] = [
                    self.mid_fs_brackets["car"][transition][interval]
        ...
        if self.clusters0 is None:
            ...
            for data_type in self.behaviour_types:
                if isinstance(self.clusters0[data_type], int):
                    self.clusters[data_type] = [
                        self.clusters0[data_type] for _ in self.homes
                    ]
                else:
    ...
    def _next_factors(self, transition, prev_clusters, homes):
        ...
        interval_f = {
            data_type: np.zeros(len(homes), dtype=int) for data_type in self.data_types
        }
        factors = {data_type: np.zeros(len(homes)) for data_type in self.data_types}
        random_f = {
            data_type: np.random.rand(len(homes)) for data_type in self.data_types
        }
        for i_home, home in enumerate(homes):
            for data_type in self.data_types:
                if self.own_der[data_type][home]:
                    if data_type == "gen" or (
                        data_type == "car" and transition == "we2wd"
                    ):
                        transition_ = "all"
                    else:
                    ...
                    if (
                        data_type == "car"
                        and prev_clusters[data_type][home]
                        == self.n_all_clusters[data_type] - 1
                    ):
                        # no trip day
                        ...
                        previous_intervals = tuple(
                            utils.f_to_interval(
                                prev_factors[data_type][home][
                                    -(self.n_consecutive_days - 1 - d)
                                ],
                                self.fs_brackets[data_type][transition_],
                            )
                            for d in range(self.n_consecutive_days - 1)
                        )
                        probabilities = self.p_pos[data_type][transition_][
                            previous_intervals
                        ]
    ...
    def _next_clusters(self, transition, prev_clusters):
        ...
        random_clus = [
            [np.random.rand() for _ in self.homes] for _ in self.behaviour_types
        ]
        for home in self.homes:
    ...
    def _transition_type(self):
        ...
        prev_day_type = "wd" if (self.date - timedelta(days=1)).weekday() < 5 else "we"
            ...
            while (
                ...
                if factors[i_home] > 0 and interval_f_car[i_home] > 0:
                    ...
                    profile = self._generate_profile(
                        "car", day_type, clusters["car"][home]
                    )
    ...
    def forward(self, generator, data_type, n_steps_nonzero, min=None, max=None):
        ...
        if data_type != "car":
            ...
            output = th.div(output, th.sum(output, dim=1).reshape(-1, 1)).reshape(
                -1, size_output
            )
    ...
    def _generate_profile(self, data_type, day_type, cluster):
        ...
        while not profile_validated and its < 1000:
            ...
            if self.clus_dist_share < 1:
                ...
                cluster_distance = fitted_kmeans_obj.transform(transformed_features)[
                    0, idx
                ]
    ...
    def _compute_number_of_available_profiles(self, data_type, day_type, i_month):
        if data_type in self.behaviour_types:
            ...
            if data_type == "car":
                for cluster in range(len(self.n_prof[data_type][day_type])):
                    assert self.n_prof[data_type][day_type][cluster] == len(
                        self.profs[data_type]["cons"][day_type][cluster]
                    ), (
    ...
    def _select_profiles(
        ...
        i_profs = []
        n_profs0 = self._compute_number_of_available_profiles(
            data_type, day_type, i_month
        )
        n_profs = n_profs0
        for home in self.homes:
            if data_type in self.behaviour_types:
            ...
            if data_type == "car":
                assert i_prof < len(
                    profs["cons"][day_type][clusters[data_type][home]]
                ), (
    ...
    def _load_dem_profiles(self, profiles, prm):
        ...
        self.n_prof["loads"] = {}
        clusters = [
            int(file[1]) for file in os.listdir(prm["profiles_path"] / "norm_loads")
        ]
        ...
        for day_type in prm["syst"]["weekday_types"]:
            ...
            self.n_prof["loads"][day_type] = [
                len(profiles["loads"][day_type][clus]) for clus in range(n_dem_clus)
            ]
            ...
            for file in files:
                if any(
                    ...
                    if check_file_start("generator", file):
                        profile_generator[f"{data_type}_{day_type}_{cluster}"] = (
                            th.load(path / file)
                        )
                    if check_file_start("zerovalues", file):
                        self.zero_values[f"{data_type}_{day_type}_{cluster}"] = np.load(
                            path / file
                        )
    ...
    def _check_charge(self, home: int, day: dict) -> bool:
        ...
        while feasible:
            ...
            if day["avail_car"][home, time_step]:  # if car is currently in garage
                ...
                t_trip = time_step
                while not end:
                    trip_load, dt_to_trip, t_end_trip = self._next_trip_details(
                        t_trip, home, day
                    )
    ...
    def _get_charge_req(
        ...
        for it in range(len(trip_loads)):
            ...
            charge_req = max(0, charge_req + trip_load - dt_to_trip * self.car["c_max"])
        return charge_req
    ...
    def _ev_unavailable_start(self, time_step, home, day):
        feasible = True
        trip_load, dt_to_trip, _ = self._next_trip_details(time_step, home, day)
        if trip_load > self.store0:
            # trip larger than initial charge
            # and straight away not available
            feasible = False
        if (
            sum(day["avail_car"][home][0:23]) == 0
            and sum(day["loads_car"][home][0:23]) > self.car["c_max"] + 1e-2
        ):
            feasible = False
        trip_load_next, next_dt_to_trip, _ = self._next_trip_details(
            dt_to_trip, home, day
        )
        if (
            next_dt_to_trip > 0
    ...
    def _next_trip_details(
        ...
        if len(ts_trips) > 0 and start_t + ts_trips[0] < self.n_steps:
            ...
            t_back = (
                int(ts_back[0]) if len(ts_back) > 0 else len(day["avail_car"][home])
            )
    ...
    def _plot_ev_avail(
        self, day_plot, avail_car_plot, hr_per_t, hours, home, cumulative_plot
    ):
        bands_ev = []
        n_steps = self.n_steps * self.it_plot if cumulative_plot else self.n_steps
        non_avail = [i for i in range(n_steps) if avail_car_plot[i] == 0]
        if len(non_avail) > 0:
        ...
        fig, ax = plt.subplots()
        ax.step(hours[0:n_steps], day_plot[0:n_steps], color="k", where="post", lw=3)
            ...
            for data_type in self.data_types:
                ...
                for home in self.homes:
                    ...
                    if cumulative_plot:
                        ...
                        for i in range(self.it_plot):
                            plt.vlines(
                                i * self.n_steps, 0, max(day_plot), ls="--", color="k"
                            )
                    fig.savefig(self.save_day_path / title)
                    plt.close("all")
    ...
    def _init_params(self, prm):
        ...
        self.own_car = (
            prm["car"]["own_car"]
            if "car" in prm and "own_car" in prm["car"]
            else np.ones(self.n_homes, dtype=bool)
        )
        self.own_PV = (
            prm["gen"]["own_PV"]
            if "gen" in prm and "own_PV" in prm["gen"]
            else np.ones(self.n_homes, dtype=bool)
        )
        self.own_loads = (
            prm["loads"]["own_loads"]
            if "loads" in prm and "own_loads" in prm["loads"]
            else np.ones(self.n_homes, dtype=bool)
        )
        self.own_der = {
            "car": self.own_car,
    ...
    def _get_transformed_features(self, profile, data_type, cluster_id):
        ...
        if data_type == "loads":
            ...
            values = [
                np.mean(
                    profile[
                        int(interval[0] * self.n_steps / 24) : int(
                            interval[1] * self.n_steps / 24
                        )
                    ]
                )
            ...
            features = profile[int(6 * 24 / self.n_steps) : int(22 * 24 / self.n_steps)]
