...
def _make_data(
    ...
    if data_source == "CLNR":
        ...
        home_data["start_no"] = home_data.apply(
            lambda x: dmy_to_cum_day(x.start_day, x.start_month, x.survey_year),
            axis=1,
        )
        home_data["end_no"] = home_data.apply(
            lambda x: dmy_to_cum_day(x.end_day, x.end_month, x.survey_year),
            axis=1,
        )
    ...
    for row in range(len(home_data)):
        ...
        if (
            data_source == "CLNR" and home_data["test_cell"][row] in valid_test_cells
        ) or data_source == "NTS":
...
def _load_data(
    save_path: Path, str_save: List[str], data_source: str, home_type: dict
