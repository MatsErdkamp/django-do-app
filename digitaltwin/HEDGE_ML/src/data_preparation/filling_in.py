...
def _number_missing_points(
    step_len: int, time: int, mins: List[int]
    ...
    if time == 0 and mins[0] != 0:
        ...
        fill_t.append(time)
    if time == len(mins) - 1 and mins[time] != 24 * 60 - step_len:
        ...
        for jump in jumps:
            ...
            for bound in bounds:
                ...
                if len(idx) == 0:
                    ...
                    vals[direction][jump][bound] = sequence[data_type][idx[0]]
            if sum_none == 0:
...
def _evaluate_missing(
    ...
    val_next = day[data_type][t_next_val] if t_next_val < len(day[data_type]) else None
    ...
    if fill_type == "lin_interp":
        ...
        replacements, types_replacement_t, differences = _potential_replacements(
...
def _check_fill_missing(
    ...
    for time in range(len(day["mins"])):
        n_miss, fill_t = _number_missing_points(prm["step_len"], time, day["mins"])
        for n_miss_, fill_t_ in zip(n_miss, fill_t):
            if n_miss_ > 1:
                ...
                if (
                    ...
                    _plot_missing_data(day, data_type, prm, missing_fig_path)
            else:
                to_fill.append(fill_t_)
                assert not (
                    to_fill == 0 and day["mins"][0] == 0
...
def _assess_filling_in(
    ...
    if (
        ...
        for fill_type in prm["fill_types"]:
            _, _, types_replaced_eval[fill_type], replaced_val = _evaluate_missing(
                prm,
                time,
                day,
                data_type,
            ...
            if replaced_val is not None:
                abs_error[fill_type] = np.append(
                    abs_error[fill_type], abs(day[data_type][time] - replaced_val)
                )
...
def fill_whole_days(
    ...
    for fill_type in prm["fill_types"]:
        types_replaced_eval[fill_type] = initialise_dict(
            prm["replacement_types"], "zero"
        )
        ...
        for time in range(len(day[data_type])):
            ...
            if prm["do_test_filling_in"]:
                if random.random() < prm["prob_test_filling_in"]:
                    types_replaced_eval, abs_error = _assess_filling_in(
                        prm,
                        day,
                        time,
                        data_type,
    ...
    if not prm["do_test_filling_in"]:
        ...
        for fill_type in prm["fill_types_choice"]:
            assert sum(types_replaced_eval[fill_type].values()) > 0, (
                f"{data_type} {fill_type} " f"types_replaced_eval {types_replaced_eval}"
            )
        for fill_type in prm["fill_types"]:
            assert len(abs_error[fill_type]) > 0, f"len(abs_error[{fill_type}]) == 0"
...
def stats_filling_in(
    ...
    for fill_type in prm["fill_types"]:
        ...
        filling_in["p99_abs_err"][fill_type] = np.percentile(
            all_abs_error[fill_type], 99
        )
        filling_in["max_abs_err"][fill_type] = np.max(all_abs_error[fill_type])
        filling_in["n_sample"][fill_type] = len(all_abs_error[fill_type])

    filling_in["share_types_replaced"] = initialise_dict(
        prm["fill_types_choice"], "empty_dict"
    )
    for fill_type in prm["fill_types_choice"]:
        sum_share = 0
        for replacement_type in prm["replacement_types"]:
            share = (
                types_replaced[fill_type][replacement_type]
