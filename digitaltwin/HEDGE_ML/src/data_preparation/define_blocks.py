...
def _get_indexes_blocks(prm, data_type, n_rows):
    ...
    with open(path, "wb") as file:
        pickle.dump(block_indexes, file)
...
def add_out(
    prm,
    out: Tuple[List, List[float], Dict[str, int], np.ndarray, List[int]],
    days: list,
...
def save_outs(outs, prm, data_type, chunks_rows):
    ...
    types_replaced = initialise_dict(prm["fill_types_choice"], "empty_dict")
    for fill_type in prm["fill_types_choice"]:
    ...
    if prm["data_type_source"][data_type] == "CLNR":
        np.save(prm["outs_path"] / f"granularities_{data_type}", granularities)
    ...
    if prm["do_test_filling_in"] and prm["data_type_source"][data_type] == "CLNR":
        stats_filling_in(prm, data_type, all_abs_error, types_replaced)
...
def get_data_chunks(prm, data_type):
    ...
    for potential_path in potential_paths:
        ...
        unique_ids_path = (
            potential_path_ / f"unique_ids_{data_type}_{prm['n_rows'][data_type]}.npy"
        )
        chunks_path = (
            potential_path_
    ...
    chunks_rows = _rows_ids_to_chunks(prm, idx, ids, data_type, chunks_path)
...
def _get_rows_ids(prm, data_type, unique_ids_path):
    ...
    current_id = None
    with open(
        prm["var_path"][data_type], newline=prm["line_terminator"][data_source]
    ) as file:
        reader = csv.reader(file, delimiter=prm["separator"][data_source])
    ...
    np.save(prm["save_other"] / f"n_ids_0_{data_id(prm, data_type)}", len(unique_ids))
