...
def _formatting(prm: dict, run_config: dict) -> Tuple[dict, dict]:
    ...
    run_config["n_consecutive_days"] = sorted(
        run_config["n_consecutive_days"], reverse=True
    )

    test_cell_keys = list(prm["test_cell_translation"].keys())
    for key in test_cell_keys:
        if not isinstance(key, str):
            prm["test_cell_translation"][str(key)] = prm["test_cell_translation"][key]
...
def _init_data_filling(prm, run_config):
    ...
    prm["jump_dt"] = {"week": 7 * 24 * 60, "day": 24 * 60, "two_days": 2 * 24 * 60}
    ...
    prm["fill_types_choice"] = [
        fill_type for fill_type in prm["fill_types"] if fill_type != "lin_interp"
    ]
    ...
    if run_config["candidate_factor_distributions"] is None:
        ...
        for candidate in run_config["candidate_distr"]:
            if candidate in distributions:
                prm["candidate_factor_distributions"][candidate] = distributions[
                    candidate
                ]
            else:
                print(
                    f"Please add {candidate} to the "
...
def _update_paths(prm: dict, run_config: dict) -> dict:
    ...
    prm["paths"]["hedge_inputs"] = (
        prm["paths"]["input_folder"]
        / prm["paths"]["hedge_inputs_folder"]
...
def _make_dirs(prm: dict):
    ...
    for folder in [f"norm_{data_type}" for data_type in prm["data_types"]] + [
        "car_avail"
    ]:
...
def get_parameters() -> Tuple[dict, dict]:
    ...
    prm["datetime_entries"] = prm["date_entries"] + prm["time_entries"]
    ...
    for data_type in prm["CLNR_types"]:
        prm["sequence_entries"][data_type] = prm["sequence_entries_CLNR"] + [data_type]
